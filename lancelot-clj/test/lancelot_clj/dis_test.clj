...
(deftest dis-test
  (let [cs (testutils/make-capstone capstone.Capstone/CS_ARCH_X86 capstone.Capstone/CS_MODE_32)
    ...
    (testing "disasm-one"
      (let [i0 (dis/disassemble-one cs buf 0x0)]
        (is (= (.-address i0) 0x0))
        (is (= (.-mnemonic i0) "push"))
        (is (= (.-opStr i0) "ebp")))
      (let [i1 (dis/disassemble-one cs buf 0x1 0x1)]
        (is (= (.-address i1) 0x1))
        (is (= (.-mnemonic i1) "mov"))
        (is (= (.-opStr i1) "ebp, esp")))
      ;; here's an overlapping instruction at offset 0x2
      (let [i2 (dis/disassemble-one cs buf 0x2 0x2)]
        (is (= (.-address i2) 0x2))
        (is (= (.-mnemonic i2) "in"))
        (is (= (.-opStr i2) "eax, -0x7d"))))

    (testing "disasm-all"
      (let [insns (into [] (map dis/format-insn (dis/disassemble-all cs buf 0x0)))]
